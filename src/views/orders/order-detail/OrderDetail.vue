...
<template>
  <div class="layout-topbar">
    <div class="layout-topbar-logo-container">
      <Button label="Pesanan" icon="pi pi-chevron-left" text severity="secondary" as="router-link" :to="{ name: 'orders' }" />
    </div>
    <div class="layout-topbar-actions">
      <div class="layout-config-menu">
        <Button icon="pi pi-download" severity="secondary" text v-tooltip.bottom="'Unduh'" />
      </div>
    </div>
  </div>
  <div class="pt-[5rem]">
    <div class="card flex flex-col md:gap-6 gap-1 md:max-w-screen-lg mx-auto">
      ...
      <div class="flex flex-col md:gap-4 gap-1">
        ...
        <div class="px-4" v-for="(order, index) in item.items" :key="index">
          ...
          <div class="flex flex-col md:gap-2 gap-1 md:mt-2 mt-1">
            <div class="flex gap-2 pl-4" v-for="(category, index) in order.category" :key="index">
              ...
              <h6 class="md:text-sm sm:text-base text-[0.4rem] m-0 font-normal w-2/12 text-end">
                Rp{{ formatPrice(category.price * category.qty) }}
              </h6>
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-2 px-4">
          ...
          <div class="w-1/2 flex flex-col md:gap-2 gap-1">
            ...
            <div class="flex justify-between gap-2">
              <p class="md:text-sm sm:text-base text-[0.4rem] m-0">Jenis Property(Rumah)</p>
              <p class="md:text-sm sm:text-base text-[0.4rem] m-0 text-end">Rp{{ formatPrice(0) }}</p>
            </div>
            <div class="flex justify-between gap-2">
              <p class="md:text-sm sm:text-base text-[0.4rem] m-0">Subtotal Harga Barang</p>
        ...
        <div class="flex gap-2 px-4 items-end">
          <p class="md:text-sm sm:text-base text-[0.4rem] m-0 w-1/2">
            Invoice ini sah dan diproses oleh komputer Silakan hubungi Tokopedia Care apabila kamu membutuhkan bantuan.
          </p>
